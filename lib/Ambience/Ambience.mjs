export default class Ambience extends HTMLElement {
  ...
  updateMediaElement() {
    console.log('updateMediaElement', this.getAttribute('for'))
    let mediaElement;

    if (this.hasAttribute('for')) {
      mediaElement = document.querySelector(`#${this.getAttribute('for')}`);
    }

    console.log('mediaElement', mediaElement);

    if (!mediaElement) {
      const selector = Ambience.MEDIA_TAGS.join(', ');
      const slot = this.#slot;
      
      mediaElement = [...slot.assignedElements()].reduce((mediaElement, element) => {
        if (!mediaElement) {
          mediaElement = element.matches(selector)
            ? element
  ...
  update() {
    ...
    this.#backdrop.style.width = `${w}`;
    this.#backdrop.style.height = `${h}`;
    this.#canvas.width = w;
